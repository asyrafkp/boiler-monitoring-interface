...
console.log('Looking for 1/21 entries in REPORT B3:\n');
const dataRows = data.filter(row => row[0] && row[0] !== 'Date' && !String(row[0]).includes('REPORT'));

dataRows.forEach((row, idx) => {
  if (String(row[0]).includes('1/21')) {
    console.log(`\nFound 1/21 at row ${idx + 1}:`);
